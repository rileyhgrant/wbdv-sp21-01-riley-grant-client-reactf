  ...
  return (
    <tr>
        ...
        {!editing && (
          <>
            <Link to={`/courses/table/edit/${course._id}`}>
              <i className="fas fa-file m-1"></i>
              {course.title}
            </Link>
            {/* TODO: Clean up this quizzes button? */}
            <Link className= "float-right m-1" to={`/courses/${course._id}/quizzes`}>
                Quizzes
            </Link>
          </>
        )}
      ...
      <td>
        {!editing && (
          <i
            onClick={() => rowFunctions.editCourse()}
            className="m-1 fas fa-edit"
          ></i>
        )}
        {editing && (
          <i
            onClick={() => rowFunctions.updateCourse()}
            className="m-1 fas fa-check"
          ></i>
        )}
        {editing && (
          <i
            onClick={() => rowFunctions.deleteCourse()}
            className="m-1 fas fa-trash"
          ></i>
        )}
      </td>
    </tr>
